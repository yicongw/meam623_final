...
def get_args():
    ...
    parser.add_argument("--gamma-threshold", type=float, default=6.8,
...
def main():
    ...
    for step_i in range(num_steps):
        ...
        if grad_gamma is not None:
            ...
            if eps < 1e-3 or prob.status != cp.OPTIMAL:
                soft = True
        ...
        if not soft:
            ...
            p.stepSimulation()
        elif grad_gamma is None:
            # No viability gradient AND QP failed/infeasible: gravity comp + damping.
            tau_full = list(p.calculateInverseDynamics(
                robot, all_pos, all_vel, [0.0] * n_all_dof))
            tau_arm = [tau_full[idx] - 5.0 * all_vel[idx]
                       for idx in arm_indices_in_all]
            p.setJointMotorControlArray(
                robot, movable_joints, p.TORQUE_CONTROL, forces=tau_arm)
            p.stepSimulation()
        ...
        # --- Collision detection (real penetration only: c[8] < -1e-4 m) ---
        # PyBullet's getContactPoints reports any pair within contactBreakingThreshold
        # (~4mm default), and even after filtering c[8] < 0 the resolver returns
        # transient -1e-5 m grazing contacts (within numerical noise).  We
        # require ≥0.1mm of actual penetration before declaring a real collision.
    ...
    csv_path = os.path.join(args.output_dir,
                            f"openarm_dual_run_{int(time.time())}{suffix}.csv")
